...
@Component
public class ResolucionNotificadaClasificacion implements EvaluadorClasificacion {

  @Autowired
  ExpedienteEstadoDao expedientesEstadoDao;
  @Autowired
  ...
  public boolean cumpleClasificacion(BigInteger idExp) throws SinacException {
    ...
    if (expEstados != null && !expEstados.isEmpty()) {
      expEstados = expEstados.stream().filter(c -> "DRNT".equals(c.getEstado().getEstadoFin().getCodEstado())).toList();
    }
    return expEstados != null && !expEstados.isEmpty();
  }

...
@Component
public class AccionCerrarExpedienteEjecutadaClasificacion implements EvaluadorClasificacion {
  ...
  public boolean cumpleClasificacion(BigInteger idExp) throws SinacException {
    ...
    List<ExpedienteEstadoEntity> lista = this.expedienteEstadoDao.getEstadosExpedienteSinFlgActivo(idExp);
    lista.forEach(estado -> listaEstados.add(this.expedienteEstadoMapper.toDto(estado)));

    return listaEstados.stream().anyMatch(e -> "EXPC".equals(e.getEstado().getEstadoFin().getCodEstado()));
  }

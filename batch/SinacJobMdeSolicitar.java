...
/**
 * Job para solicitar al Mde los informes en estado pendiente
 */
@Component
public class SinacJobMdeSolicitar extends SinacJob<BigInteger> {

  private static final Logger LOG = LoggerFactory.getLogger(SinacJobMdeSolicitar.class);
  ...
  public List<BigInteger> recuperarItems(Map<String, Object> contextData) {
    LOG.info("SinacJobMde.recuperarItems - Init");
    ...
    List<BigInteger> lista = expedientesFacade.getIdsExpedienteInformesByCodEstadoCodTipoInforme(
        Constantes.EstadosInforme.ESTADO_INFORME_PENDIENTE, Constantes.TiposInforme.TIPO_INFORME_DEF);
    LOG.info("Tamaño lista: {}", lista.size());
    LOG.info("SinacJobMde.recuperarItems - End");
    return lista;
  }
  ...
  public void procesarItem(BigInteger item, Map<String, Object> contextData) {
    try {
      LOG.info("SinacJobMde.procesarItem - Init");
      LOG.info("Item: {}", item);
      Map<String, Object> valores = new HashMap<>();
      ExpedienteDto expediente = expedientesFacade.getExpedienteByIdExpedienteInforme(item);
      valores.put("expediente", expediente);
      valores.put("idExp", expediente.getIdExp());
      valores.put("idUsu", usuariosService.getUsuarioByUsuarioJusticia(getUsuarioJusticia()).getIdUsu());
      valores.put("flgProceso", true);
      valores.put("tipoInformeMde", Constantes.TiposInforme.TIPO_INFORME_DEF);
      valores.put("idPro", expediente.getProcedimientoDto().getIdPro());
      ...
      Long idProFaseTraOpeAcc = expedientesFacade.getIdProcedimientosFasesTramitesOperacionesAccionesByCodProTraOpeAcc(
          expediente.getProcedimientoDto().getCodPro(), "INF", "IMDE", "SMDE");
      expedientesFacade.ejecutarAccion(idProFaseTraOpeAcc, valores);

      LOG.info("SinacJobMde.procesarItem - End");
